...
const chatFunction = require("../middlewares/chat");
...
const router = express.Router();
    ...
    router.get("/api/getChats", (req, res) => {
        chatFunction.getChats(req, result => res.send(result));
    });
    ...
    router.post("/api/getHistory", (req, res) => {
        if (req.body) {
            ...
            mongoRequests.getHistory(id, (err, result) => {
    ...
    router.get("/api/findChats", (req, res) => {
        mongoRequests.findChats((err, result) => {
            if (err) return console.log(err);
            res.send(result)
        });
    });
    ...
    exports.renderIndex = (req, res) => {
        ...
        if (req.headers.cookie && req.headers.cookie.indexOf('token') > -1) {
            ...
            mongoRequests.checkToken(token, (result) =>{
                if (!result.error) {
                    if (isMobile) {
                        res.render("./mobile/index.jade", {title: "Chatty", isMobile: true})
                    } else {
                        res.render("./web/index.jade", {title: "Chatty", isMobile: false})
                    }
                } else {
                    if (isMobile) {
                        res.render("./web/login.jade", {title: "Chatty", isMobile: true})
                    } else {
                        res.render("./web/login.jade", {title: "Chatty", isMobile: false})
                    }
                }
            });
        }
